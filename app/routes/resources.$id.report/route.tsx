...
export const action: ActionFunction = async ({ request, context, params }) => {
    ...
    if (!reportType || !user.auth || !id)
        return null;

    await db
        .insert(resourceReport)
        .values({
            category: reportType,
            resource: Number(id),
            reporter: user.id,
...
function ReportRadio(configs: ReportRadioArgs) {
    return <div className="form-control m-2 p-3 border border-neutral-content/30 rounded-lg peer-checked:border-blue-500">
        ...
        <label className="label cursor-pointer peer flex flex-col">
            <div className="w-full flex flex-row justify-between items-center">
                ...
                <input type="radio" name="reportType" className="radio" value={configs.value} />
            </div>
            <div className="card bg-base-100 p-5 w-full mt-5">
                <p className="text-sm text-neutral-content mb-0">{configs.children}</p>
            </div>
        </label>
    </div>;
}
        ...
        {fetcher.data?.success !== true
            ? <fetcher.Form method="post">
                ...
                <ReportRadio name="版權疑慮" value={ReportType.Copyright}>
                    如果您認為此資源侵犯了您的版權，請選擇此選項。Umira Projects 非常重視您的權益，並會在第一時間進行處理。
                </ReportRadio>
                <ReportRadio name="不正確的資訊" value={ReportType.Incorrect}>
                ...
                {parentData?.auth
                    ? <button className="btn btn-primary w-full">提交檢舉</button>
                    : <button onClick={() => navigate("/login?return=" + encodeURIComponent(location.pathname))} className="btn w-full">請先登入至東華資源庫</button>}
            </fetcher.Form>
            : <div>
                <h2>感謝您的檢舉</h2>
